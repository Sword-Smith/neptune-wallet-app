...
export function TrashModal({ opened, close }: { opened: boolean; close: () => void }) {
  ...
  const rows =
    cacheFiles && cacheFiles.map((item, index) => <ChacheFileItem key={index} item={item} />);
  return (
    <Modal
      opened={opened}
      size="auto"
...
export function ChacheFileItem({ item }: { item: BlockCacheFile }) {
  ...
  return (
    <Table.Tr>
      <Table.Td>{item.network == "main" ? "Mainnet" : "Testnet"}</Table.Td>
      <Table.Td>
