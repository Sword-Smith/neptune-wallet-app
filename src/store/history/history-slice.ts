...
>("/api/history/queryActivityHistory", async ({ serverUrl, addressId, historyType = "All" }) => {
...
async function handleTransaction(data: HistoryData[], addressId: number, historyType: string) {
  ...
  newQueryHistory &&
    newQueryHistory.forEach((element) => {
      ...
      if (hasTxId) {
        let findHistoryByID =
          ...
          localHistory.find((item) => item.txid === element.txid);
        if (findHistoryByID) {
